			...
			<div class="SP_information">
				...
				<div class="SP_infoMegBox_wrap">
				 <sec:authentication property="principal" var="pinfo" />
					<p class="ment" style="margin-bottom: 12px"><strong><span><span class="xans-member-var-name">${pinfo.user.name}</span></span></strong>님 반갑습니다.</p>
					<p class="grade">회원님의 등급은 <!--img src="" alt="" class="myshop_benefit_group_icon_tag" / --><strong class="group">[<span class="groupName" id="groupName"><span class="xans-member-var-group_name">${pinfo.user.memid == 1 ? 'friend' : pinfo.user.memid == 2 ? 'family' : pinfo.user.memid == 3 ? 'crew' : ''}</span><span class="myshop_benefit_ship_free_message"></span></span>]</strong>입니다. </p>
					<p class="gradeValue  gradeGuide" id="sGradeAutoDisplayArea">다음 등급까지 
					<span id="changePrd" class="price  sAutoGradeDisplay">
					<span class="xans-member-var-sGradeIncreasePrice">150,000</span>원</span> 남았습니다.</p>
...
<li>
						<div class="SP_displayTable">
							<div class="SP_displayTableInr">
								...
								<div class="SP_myOrderInfoCont"><a href="/myshop/mileage/historyList.html" class="SP_mileageCount" style=""><fmt:formatNumber value="${pinfo.user.userpoint}"/></a></div>
							</div>
						</div>
					</li>
					<li class="">
						<div class="SP_displayTable">
							<div class="SP_displayTableInr">
								...
								<div class="SP_myOrderInfoCont"><a href="/myshop/coupon/coupon.html">${couponList != null ? couponList.size() : 0}</a></div>
							</div>
						</div>
					</li>
				</ul>
						        ...
						        <c:when test="${not empty couponList}">
						            <c:forEach var="coupon" items="${couponList}">
						                <tr>
						                    ...
						                    <td class="left">${coupon.cpninfo}</td>
						                    
						                    <td>${coupon.cpndiscountrate}%</td>
						              
						                    <td>-</td>
						                    
						                    <td>
						                        <fmt:formatNumber value="${coupon.cpnconvalue}" type="number" maxFractionDigits="0" />
						                    </td>
						                   
						                    <td>
						                        <c:choose>
						                            <c:when test="${coupon.cpnapply == 'D'}">배송비</c:when>
						                            <c:otherwise>상품</c:otherwise>
						                        </c:choose>
						                    </td>
						                    
						                    <td class="left useDate">
						                        <fmt:formatDate value="${coupon.cpnstartdate}" pattern="yyyy-MM-dd" /> ~ 
