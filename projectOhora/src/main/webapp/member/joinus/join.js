...
function checkDuplication(type, inputId, msgId) {
    ...
    $.ajax({
        url: `/checkDuplicate.ajax?type=${type}&value=${value}`, 
        type: 'GET',
        dataType: 'json',
        beforeSend:function(xhr){
               console.log(">beforeSend.add()............. ");
          },
...
function sendAuthCode(phone) {
    ...
    $.ajax({
        url: '/sendAuthCode.ajax', // Spring Controller URL
        type: 'GET',
        data: { phone: phone },
        dataType: 'json', 
        beforeSend: function(xhr) {
...
function verifyAuthCode() {
    ...
    console.log("verifyAuthCode 함수 시작"); // 실행 여부 확인
    const authCode = document.getElementById('authCode').value;
    const authMsg = document.getElementById('authMsg');
    
    console.log("authCode 값:", authCode);
  ...
  //  console.log("CSRF Token Value:", csrfTokenValue);
      ...
      if (!authCode) {
        console.error("authCode 값이 비어있습니다.");
        return;
    }  

    $.ajax({
        url: '/verifyAuthCode.ajax', 
        type: 'POST',
         headers: {
        ...
        error: function(xhr, status, error) {
		    ...
		    console.error("응답 상태 코드:", xhr.status); // HTTP 상태 코드
		    console.error("응답 메시지:", xhr.responseText); // 서버 응답 내용
